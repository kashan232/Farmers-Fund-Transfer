...
class AgricultureFarmerRegistrationController extends Controller
{
    ...
    public function store_agri_farmers(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            // Extract data arrays from the request
            $titleNames = $request->input('title_name', []);
            $titleCnics = $request->input('title_cnic', []);
            $titleNumbers = $request->input('title_number', []);
            $titleAreas = $request->input('title_area', []);

            // Validate if any of the arrays are empty
            if (empty($titleNames) || empty($titleCnics) || empty($titleNumbers) || empty($titleAreas)) {
                return redirect()->back()->withErrors(['Error: Arrays cannot be empty.']);
            }
            ...
            try {
                ...
                // Loop through arrays and store each set of data
                for ($i = 0; $i < $count; $i++) {
                    AgricultureFarmersRegistration::create([
                        ...
                        'title_cnic' => $titleCnics[$i],
                        'title_number' => $titleNumbers[$i],
                        'title_area' => $titleAreas[$i],
                        'crops' => $request->crops,
